...
public class King extends Piece{
    ...
    protected Set<Position> getPositions(Board board, Position position) {
        ...
        if(colPlusOne < 8){
            ...
            addPositionIfvalid(ret, board, new Position(colPlusOne, lin));
        }
        if(colMinusOne >= 0){
            columns.add(colMinusOne);
            addPositionIfvalid(ret, board, new Position(colMinusOne, lin));
        }

        var lines = new ArrayList<Integer>();
        if(linPlusOne < 8){
            lines.add(linPlusOne);
            addPositionIfvalid(ret, board, new Position(col, linPlusOne));
        }
        if(linMinusOne >= 0){
            lines.add(linMinusOne);
            addPositionIfvalid(ret, board, new Position(col, linMinusOne));
        }
        for(var column : columns){
            for(var line : lines){
                addPositionIfvalid(ret, board, new Position(column, line));
            }
        }
        return ret;
    }
